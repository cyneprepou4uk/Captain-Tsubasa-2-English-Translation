...
loc_0x027010:
C D 1 - - - 0x027010 13:B000: A9 00     LDA #$00
...
loc_B02D_loop:
C D 1 - - - 0x02703D 13:B02D: A4 8A     LDY ram_008A
...
C - - - - - 0x027060 13:B050: AD 15 05  LDA ram_0515_buffer_flag
C - - - - - 0x027063 13:B053: D0 F6     BNE bra_B04B_loop
C - - - - - 0x027065 13:B055: A9 01     LDA #$01
C - - - - - 0x027067 13:B057: 8D 15 05  STA ram_0515_buffer_flag
...
C - - - - - 0x0270BB 13:B0AB: 8D 15 05  STA ram_0515_buffer_flag
C - - - - - 0x0270BE 13:B0AE: 60        RTS



loc_B0AF_скрытие_букв_спрайтами:
C D 1 - - - 0x0270BF 13:B0AF: A4 8A     LDY ram_008A
C - - - - - 0x0270C1 13:B0B1: B1 88     LDA (ram_0088),Y
C - - - - - 0x0270C3 13:B0B3: C9 E0     CMP #$E0
C - - - - - 0x0270C5 13:B0B5: 90 01     BCC bra_B0B8_00_DF
; E0-FF
...
C - - - - - 0x027117 13:B107: 8D 15 05  STA ram_0515_buffer_flag
                                        LDA #$00
...
C D 1 - - - 0x027349 13:B339: A5 8A     LDA ram_008A
...
- D 1 - I - 0x0271AE 13:B19E: 33 B3     .word ofs_020_B333_E7_закончить_чтение_строки
...
loc_B20F:
C D 1 - - - 0x02721F 13:B20F: 20 0C C5  JSR sub_0x03CD8C_получить_адрес_игрока
...
- D 1 - I - 0x02723D 13:B22D: 3E B2     .word ofs_021_B23E_00_осветление_экрана
- D 1 - I - 0x027241 13:B231: A6 B2     .word ofs_021_B2A6_01_затемнение_экрана
- D 1 - I - 0x027243 13:B233: DB B2     .word ofs_021_B2DB_02_отображение_текста
...
C - - - - - 0x0272BF 13:B2AF: 85 3A     STA ram_003A_temp
...
C - - - - - 0x0272CD 13:B2BD: 05 3A     ORA ram_003A_temp
C - - - - - 0x0272CF 13:B2BF: A4 3A     LDY ram_003A_temp
...
C - - - - - 0x0272E3 13:B2D3: A5 3A     LDA ram_003A_temp
...
loc_B2F7:
C D 1 - - - 0x027307 13:B2F7: A2 00     LDX #$00
bra_B2F9_loop:
...
C - - - - - 0x02737E 13:B36E: A5 20     LDA ram_for_2000
C - - - - - 0x027380 13:B370: 29 FC     AND #$FC
C - - - - - 0x027382 13:B372: 85 20     STA ram_for_2000
...
bra_B3CB_loop:
loc_B3CB_loop:
C D 1 - - - 0x0273DB 13:B3CB: A9 01     LDA #$01
...
C D 1 - - - 0x02740A 13:B3FA: A9 01     LDA #$01
...
C - - - - - 0x027422 13:B412: AD 15 05  LDA ram_0515_buffer_flag
C - - - - - 0x027425 13:B415: D0 F6     BNE bra_B40D_ожидание_освобождения_буфера
C - - - - - 0x027427 13:B417: A9 01     LDA #$01
C - - - - - 0x027429 13:B419: 8D 15 05  STA ram_0515_buffer_flag
...
C - - - - - 0x027449 13:B439: 8D 15 05  STA ram_0515_buffer_flag
...
con_E3_tecmo_logo           = $E3   ; показать зайца и логотип
con_E4_offset               = $E4   ; поменять начальный адрес ppu для вывода текста
con_E5_brightness           = $E5   ; управление яркостью и отображением текста
    con_light_on                = $00 ; осветление экрана
    con_light_off               = $01 ; затемнение экрана
    con_show_txt                = $02 ; показать текст (используется после предварительной записи текста в ppu)
...
con_E7_endline              = $E7   ; закончить чтение текста
    ...
    .byte con_E5_brightness, con_light_off
    ...
    .byte con_E5_brightness, con_light_off
    .byte con_E3_tecmo_logo



.out .sprintf("Free bytes in bank 93: 0x%04X [%d]", ($C000 - *), ($C000 - *))
